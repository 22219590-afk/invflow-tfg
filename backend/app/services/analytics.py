...
class AnalyticsService:
    ...
    def forecast_product(self, product_id: int):
        """
        Calculate Holt-Winters and ARIMA forecasts for a product.
        Returns historical data, forecasts from both models, and best model info.
        """
        # TFG Fix: Use internal ID (p.id) as sent by frontend REST params
        product = self.session.exec(select(Product).where(Product.id == product_id)).first()
        if not product: return {"error": "Producto no encontrado"}
        
        # TFG Fix: Use relationship to ensure we get moves linked by odoo_id correctly
        moves = product.moves
        
        results = {"product_id": product.id, "odoo_id": product.odoo_id, "name": product.name}
        
        if not moves:
            # Generate synthetic history based on manual_daily_demand for demo/TFG
            d = product.manual_daily_demand if product.manual_daily_demand else 10.0
            sigma = product.manual_demand_std_dev if product.manual_demand_std_dev else d * 0.2
            
            import random
            random.seed(product.id)
            history = [max(0, round(random.normalvariate(d * 30, sigma * 5), 2)) for _ in range(12)]
            df = pd.Series(history, index=pd.date_range(end=datetime.now(), periods=12, freq='ME'))
        else:
            df = pd.DataFrame([{
                "date": m.date,
                "qty": m.product_uom_qty,
            } for m in moves if m.move_type == 'out'])
            df["date"] = pd.to_datetime(df["date"])
            df = df.set_index("date").resample("ME")["qty"].sum().fillna(0)
            
            if len(df) < 3: 
                history = df.tolist()
                while len(history) < 12: history.insert(0, 0.0)
                df = pd.Series(history, index=pd.date_range(end=datetime.now(), periods=12, freq='ME'))
        ...
        def calculate_mape(y_true, y_pred):
            ...
            mask = y_true > 0
            if not np.any(mask): return 20.0 # Default if no data
            return float(np.mean(np.abs((y_true[mask] - y_pred[mask]) / y_true[mask])) * 100)

        try:
            # Holt-Winters
            hw_model = ExponentialSmoothing(df, seasonal_periods=4, trend='add', seasonal='add').fit()
            hw_forecast = hw_model.forecast(6).tolist()
            results["hw_mape"] = round(calculate_mape(df, hw_model.fittedvalues), 2)
            results["holt_winters"] = hw_forecast
        except:
            results["holt_winters"] = (df.mean() * np.ones(6)).tolist()
            results["hw_mape"] = 99.0
            
        try:
            # ARIMA (1,1,1)
            arima_model = ARIMA(df, order=(1, 1, 1)).fit()
            arima_forecast = arima_model.forecast(6).tolist()
            results["arima_mape"] = round(calculate_mape(df, arima_model.fittedvalues), 2)
            results["arima"] = arima_forecast
        except:
            results["arima"] = (df.mean() * 1.1 * np.ones(6)).tolist()
            results["arima_mape"] = 99.0

        # Selection logic
        if results["arima_mape"] < results["hw_mape"]:
            results["best_model"] = "ARIMA"
            results["forecast_final"] = results["arima"]
            results["error_final"] = results["arima_mape"]
        else:
            results["best_model"] = "Holt-Winters"
            results["forecast_final"] = results["holt_winters"]
            results["error_final"] = results["hw_mape"]

        # --- MRP Grid Calculation (12 Months) ---
        current_stock = sum(q.quantity for q in product.quants)
        
        # Extended forecast for 12 months
        forecast_12 = list(results["forecast_final"])
        if len(forecast_12) < 12:
            avg = sum(forecast_12) / len(forecast_12) if forecast_12 else 10
            forecast_12.extend([avg] * (12 - len(forecast_12)))
        
        # Mock historical data from "same period last year" for comparison
        import random
        random.seed(product.id + 1000)
        last_year_sales = [max(0, round(f * random.uniform(0.8, 1.2), 2)) for f in forecast_12]
        ...
        curr = current_stock
        lead_time_months = max(1, round(product.lead_time_days / 30))
        
        pending_arrivals = {} # month_index: quantity
        
        for i in range(12):
            dem = forecast_12[i]
            # 1. To Receive
            arrival = pending_arrivals.get(i, 0)
            to_receive.append(arrival)
            
            # 2. Stock Start
            stock_start = curr + arrival
            
            # 3. Stockout
            so_days = 0
            if stock_start < dem:
                so_days = round((dem - stock_start) / (dem / 30)) if dem > 0 else 0
                so_days = min(30, so_days)
            stockout_days.append(so_days)
            
            # 4. End stock
            curr = max(0, stock_start - dem)
            projected_stock.append(curr)
            
            # 5. Stock Necessary (TFG Logic: SS + Demanda en Lead Time)
            nec = product.safety_stock + (dem * lead_time_months)
            stock_necessary.append(round(nec, 2))
            
            # 6. Ordering Plan
            order_qty = 0
            if curr < product.safety_stock:
                # Wilson EOQ or Periodic Target
                if product.abc_class == 'A':
                    order_qty = product.eoq or (dem * 1.5)
                else:
                    # Periodic Target S = d*(R+L) + SS
                    order_qty = (dem * (product.review_period/30 + lead_time_months)) + product.safety_stock - curr
                
                arrival_month = i + lead_time_months
                if arrival_month < 12:
                    pending_arrivals[arrival_month] = pending_arrivals.get(arrival_month, 0) + order_qty
            ordering_plan.append(round(order_qty, 2))
            
            # 7. Revenue
            revenue.append(round(dem * product.list_price, 2))
            
        results["mrp_grid"] = {
            "months": [(datetime.now() + timedelta(days=30*i)).strftime("%b %y") for i in range(12)],
            "last_year_sales": last_year_sales,
            "forecast": [round(f, 2) for f in forecast_12],
            "revenue": revenue,
            "on_hand": [round(s, 2) for s in projected_stock],
            "stock_necessary": stock_necessary,
            "stockout_days": stockout_days,
            "ordering_plan": ordering_plan,
            "to_receive": [round(r, 2) for r in to_receive]
        }
